...
async def receive_email(request: Request, db: Session = Depends(get_session)):
    ...
    try:
        ...
        if sns_message.get("Type") == "SubscriptionConfirmation":
            ...
            if subscribe_url:
                ...
                if response.status_code == 200:
                    ...
                    raise HTTPException(
                        status_code=400, detail="Failed to confirm SNS subscription"
                    )
        ...
        # Handle email notification
            ...
            payload = EmailPayload(
                ...
                timestamp=datetime.fromisoformat(ses_mail.get("timestamp", ""))
                if ses_mail.get("timestamp")
                else None,
            )
            ...
            email_data = get_email(
                raw_message={
                    ...
                    "message_id": ses_mail.get(
                        "messageId", f"FWD-{datetime.utcnow().timestamp()}"
                    ),
                }
            )
            ...
            if email_record:
                ...
                return {
                    "status": "skipped",
                    "message": "Email already exists or could not be processed",
                }

        else:
            raise HTTPException(
                status_code=400,
                detail=f"Unsupported SNS message type: {sns_message.get('Type')}",
            )
